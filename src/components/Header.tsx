  ...
  return (
    <header className="w-full p-4 absolute top-0 z-50 bg-white md:bg-transparent">
      ...
      <div className="flex justify-between max-w-[1200px] mx-auto items-center px-4 md:px-8">
        ...
        <div className="text-[#4b0005] md:text-white font-bold text-2xl">
           KIRAN
           <p className="text-xs text-[#4b0005] md:text-white">FASHION BLOG</p>
        </div>
        {/* Navbar Links for large screens */}
        <nav className={`hidden md:flex space-x-8 items-center ${menuOpen ? 'block' : 'hidden'}`}>
          <Link href="/" className="text-white hover:text-[#c9184a]">Home</Link>
          <Link href="/about" className="text-white hover:text-[#c9184a]">About</Link>
          <Link href="/blog" className="text-white hover:text-[#c9184a]">Blog</Link>
          <Link href="/contact" className="text-white hover:text-[#c9184a]">Contact</Link>
